...
export class StatusPageClient {
  private readonly statusPageUrl = 'https://status.protekweb.com/';

  async scrapeStatusPage(): Promise<StatusPageResponse> {
    try {
      ...
      while ((serviceMatch = serviceRegex.exec(html)) !== null) {
        ...
        const status: 'up' | 'down' | 'degraded' = uptime === '100.000%' ? 'up' : 'degraded';
